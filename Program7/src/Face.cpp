...
void Face::addTextureCoords(double s, double t){
   textureCoords.push_back(TextureCoords(s, t));
}
...
void Face::render(Pixel* pix, Matrix* zBuffer, Matrix* W, Matrix* transform, Color* mat,
                  ...
                  Texture* tex, unsigned char* textureColors)
{
   ...
   renderFace(pix, W, transform, zBuffer, v1, v2, v3, mat, lightPoint, ambLight, E, attn, shine, tex, textureColors);
...
void Face::renderFace(Pixel* pix, Matrix* W, Matrix* transform, Matrix* zBuffer, Vertex* v1, Vertex*v2, Vertex* v3,
    ...
    //Calculate l.n in world space. Interpolated later
    double ldotn1 = calculateLighting(v1, transform, lightPoint);
    double ldotn2 = calculateLighting(v2, transform, lightPoint);
    double ldotn3 = calculateLighting(v3, transform, lightPoint);
        ...
        for(int x = xMin; x <= xMax; ++x){
            ...
            if(alpha > 0.0 && beta > 0.0 && sum <= 1.0){
                ...
                if(z > zBuffer->getElement(y, x) && z <= 1){
                    ...
                    double ph = 1.0 / v1->getH();
                    double qh = 1.0 / v2->getH();
                    double rh = 1.0 / v3->getH();
                    double h = ph + (alpha * (qh - ph)) + (beta * (rh - ph)); //Actually 1/h

                    double ldotn = 10 * h * ((ldotn1 * ph) + (alpha * ((ldotn2 * qh) - (ldotn1 * ph)))
                                                           + (beta * ((ldotn3 * rh) - (ldotn1 * ph))));
                    
                    double s = ((textureCoords[0].s * ph) + (alpha * ((textureCoords[1].s * qh) - (textureCoords[0].s * ph))) 
														  + (beta  * ((textureCoords[2].s * rh) - (textureCoords[0].s * ph))));
				    double t = ((textureCoords[0].t * ph) + (alpha * ((textureCoords[1].t * qh) - (textureCoords[0].t * ph))) 
														  + (beta  * ((textureCoords[2].t * rh) - (textureCoords[0].t * ph))));

                    double s_prime = s / h;
                    double t_prime = t / h;

                    int t_texel = tex->getHeight() - (((int)(tex->getHeight() * t_prime)) % tex->getHeight()) - 1;
                    int s_texel = ((int)(tex->getWidth() * s_prime)) % tex->getWidth();

                    int index = (t_texel * tex->getWidth() + s_texel) * 3;

                    double denom = 1.0 / 255.0;
                    double r = textureColors[index] * denom * ldotn;
                    double g = textureColors[index+1] * denom * ldotn;
                    double b = textureColors[index+2] * denom * ldotn;

                    //Use to test without lighting
                    //double r = textureColors[index] * denom;
                    //double g = textureColors[index+1] * denom;
                    //double b = textureColors[index+2] * denom;
...
double Face::calculateLighting(Vertex* point, Matrix* transform, Light* lightPoint){
   Vector* normal = point->averageNormal(transform);
   Vector* temp = normal;
   normal = normal->norm();
   delete temp;

   Vertex* transformed_point = point->multiply(transform);

   Vector* l = transformed_point->sub(lightPoint->getLocation());
   temp = l;
   l = l->norm();
   delete temp;
